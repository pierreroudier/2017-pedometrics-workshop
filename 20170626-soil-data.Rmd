...
Soil data has its specificities, but generally speaking we can get away with treating it like a spatial dataset for a significant part of the analysis workflow. This is why in the first part of this tutorial, we will present tools to read and process spatial data. However in the second part we will address data structures and packages that are specific to the soil profile.
...
Spatial data can be read using the `st_read` command (it's using GDAL/OGR behind the scenes). Unlike `readOGR` from the `rgdal` package, generally the command can guess which driver it should use. 
...
Very roughly, he `sf` package provides two types of functions:
...
`sf` is very well integrated with `dplyr` and the `tidyverse`, providing verbs that are adapted to working with `geometries`. For example `summarise` will also operate a spatial union:
...
# Projection
nc_p <- st_transform(nc, crs = "+init=epsg:32119")
